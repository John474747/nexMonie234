      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="flex flex-col items-center">
          ...
          <div className="text-[15px] font-medium text-muted-foreground tracking-tight flex items-center gap-1">
            initializing <span className="font-light opacity-60">nex</span><span className="font-bold">Monie</span>...
          </div>
        </div>
      </div>
    );
        ...
        <Card className="w-full max-w-md p-10 border-none shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] rounded-[48px] bg-white">
          <div className="flex flex-col items-center mb-10 text-center">
            ...
            <h1 className="text-[26px] font-light text-[#1A1A1A] tracking-tight leading-tight">
              {isSignUp ? (
                <>create your <span className="font-light opacity-60">nex</span><span className="font-bold">Monie</span> account</>
              ) : (
                <>welcome to <span className="font-light opacity-60">nex</span><span className="font-bold">Monie</span></>
              )}
            </h1>
            <p className="text-[13px] text-gray-400 mt-4 font-medium leading-relaxed max-w-[240px]">
              Tap below to instantly explore the elite financial command center.
            </p>
          </div>

          {error && (
            <Alert variant="destructive" className="mb-6 rounded-[24px] border-red-50 bg-red-50/50">
              <AlertCircle className="h-4 w-4" />
              <AlertTitle className="font-bold text-xs uppercase tracking-widest">Security Alert</AlertTitle>
          ...
          {message && (
            <Alert className="mb-6 rounded-[24px] border-primary/20 bg-primary/5 text-primary">
              ...
              <AlertTitle className="font-bold uppercase tracking-widest">Magic Link Sent</AlertTitle>
          ...
          <Tabs defaultValue="password">
            <TabsList className="grid w-full grid-cols-2 mb-8 rounded-[20px] h-14 bg-gray-50/80 p-1.5 border border-gray-100/50">
              <TabsTrigger value="password" disabled={authLoading} className="rounded-[16px] font-semibold data-[state=active]:bg-white data-[state=active]:shadow-sm">
                <Key size={16} className="mr-2" /> Password
              </TabsTrigger>
              <TabsTrigger value="magic" disabled={authLoading} className="rounded-[16px] font-semibold data-[state=active]:bg-white data-[state=active]:shadow-sm">
            ...
            <TabsContent value="password">
              <form onSubmit={handleEmailAuth} className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="email" className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-1">Email Address</Label>
                  <Input 
                    id="email" 
                    type="email" 
                ...
                <div className="space-y-2">
                  <Label htmlFor="password" className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-1">Secure Password</Label>
                  <Input 
                    id="password" 
                    type="password" 
                    value={password} 
                ...
                <Button 
                  ...
                  disabled={authLoading}
                >
                  {authLoading ? <Loader2 className="animate-spin" /> : (isSignUp ? 'Launch Account' : 'Sign In Now')}
                </Button>
              </form>
            </TabsContent>
            ...
            <TabsContent value="magic">
              <div className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="magic-email" className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-1">Email Address</Label>
                  <Input 
                    id="magic-email" 
                    type="email" 
                    placeholder="Enter your email" 
                ...
                <Button 
                  ...
                  disabled={authLoading}
                >
                  {authLoading ? <Loader2 className="animate-spin" /> : 'Request Magic Link'}
                </Button>
              </div>
            </TabsContent>
          </Tabs>
